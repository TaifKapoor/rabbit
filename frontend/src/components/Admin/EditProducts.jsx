...
const EditProducts = () => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!dataToSend.name || !dataToSend.description || !dataToSend.price ||
            !dataToSend.countInStock || !dataToSend.sku ||
            dataToSend.sizes.length === 0 || dataToSend.colors.length === 0) {
            alert("Please fill all required fields: Name, Description, Price, Count In Stock, SKU, Sizes, Colors");
            return;
        }

        console.log("Sending data to update:", dataToSend);
        dispatch(updateProduct({ id, productData: dataToSend }));
